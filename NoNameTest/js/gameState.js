...
function GetSaveGame() {
    ...
    saveData.sd = Date.now();
    saveData.tgt = totalGameTime;
    saveData.re = [];
    saveData.bu = [];
    saveData.ma = [];
    saveData.qu = [];
...
function LoadSaveGame(saveData) {
    if (saveData.version == "1") {
        ...
        // Give Time mana
        var secondsDelta = (Date.now() - saveData.sd) / 1000;
        timeEssenceAfterLoad = Math.floor(secondsDelta / (60 * 4));
        getResourceFromId(RESOURCE_TIMEESSENCE).addAmount(timeEssenceAfterLoad);
    }
